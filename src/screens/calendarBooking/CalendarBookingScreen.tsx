...
const getSlotsCoveredByAppointment = (
  startTime: string,
  ...
  const validateForm = (): boolean => {
    ...
    if (!selectedTime) {
      ...
      if (conflictingSlots.length > 0) {
        newErrors.time =
          `This time slot conflicts with an existing appointment. The following slots are already booked: ` +
          `${conflictingSlots.join(', ')}. Please select another time.`;
      }
    }
